...
for mm in range(4):
    ...
    if USESH:
        thisValues1 = clm1.expand(
            lon=thisCentroidsLL_DEG[:, 0], lat=thisCentroidsLL_DEG[:, 1])
        thisValues2 = clm2.expand(
            lon=thisCentroidsLL_DEG[:, 0], lat=thisCentroidsLL_DEG[:, 1])
    else:
        thisValues1 = np.zeros(numCells)
        thisValues2 = np.zeros(numCells)
        for cc in range(numCells):
            thisValues1[cc] = test1(
                thisCentroidsLL_RAD[cc, 0], thisCentroidsLL_RAD[cc, 1])
            thisValues2[cc] = test2(
                thisCentroidsLL_RAD[cc, 0], thisCentroidsLL_RAD[cc, 1])
    ...
    for cc in range(numCells):
        ...
        if USESH:
            thisAvgs1[cc] = computeAreaIntegral(
                clm1, aCell, order, True, False)
            thisAvgs2[cc] = computeAreaIntegral(
                clm2, aCell, order, True, False)
        else:
            thisAvgs1[cc] = computeAreaIntegral(
                test1, aCell, order, True, False)
            thisAvgs2[cc] = computeAreaIntegral(
                test2, aCell, order, True, False)
...
if byStandardNorms:
    ...
    for diff in differences:
        ...
        L1_t1, L2_t1, Li_t1 = computeStandardNorms(
            meshCells[ii], values[ii][0], references[ii][0], areas[ii], None, False)
        L1_t2, L2_t2, Li_t2 = computeStandardNorms(
            meshCells[ii], values[ii][1], references[ii][1], areas[ii], None, False)
    ...
    for diff in differences:
        ...
        # Compute L1 Norm
        L1_test1.append(diff[0].dot(areas[ii]) /
                        (references[ii][0]).dot(areas[ii]))
        L1_test2.append(diff[1].dot(areas[ii]) /
                        (references[ii][1]).dot(areas[ii]))

        # Compute L2 Norm
        L2_test1.append(np.power(diff[0], 2.0).dot(
            areas[ii]) / (np.power(references[ii][0], 2.0)).dot(areas[ii]))
        L2_test2.append(np.power(diff[1], 2.0).dot(
            areas[ii]) / (np.power(references[ii][1], 2.0)).dot(areas[ii]))
