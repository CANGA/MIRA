...
for mm in range(4):
    ...
    thisGradValues = computeGradientFV3(
        values[mm], meshCells[mm], meshCoords[mm], stencils[mm])
    # Get gradients of cell-averaged sampled data
    thisGradAvgs = computeGradientFV3(
        references[mm],
        meshCells[mm],
        meshCoords[mm],
        stencils[mm])
...
for diff in grad_differences:
    ...
    H1, H1_2 = computeGradientPreserveMetrics(
        meshCells[ii], gradientsOnTM, varsOnTM, areas[ii], areas[ii], False)
