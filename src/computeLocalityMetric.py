...
def computeLocalityMetric(varSS, varST, varConS,
                          varCoordS, varConT, varCoordT):
    ...
    # Initialize the columns of the R*E operator (see eq. 4 for E as a scaling
    # operator)
    ...
    for ii in range(NS):
        for jj in range(NT):
            ...
            dij = 0.0
            # Get the regridded "delta field" E_i^s (NOT POSSIBLE WITHOUT R
            # MAP)
            Esi = 0.0
        ...
        L_DF = computeGlobalWeightedIntegral(
            NT, varConT, varCoordT, varDiffT, areas)

        # Compute integral of the target sampled ST data
        L_ST = computeGlobalWeightedIntegral(
            NT, varConT, varCoordT, varST, areas)
