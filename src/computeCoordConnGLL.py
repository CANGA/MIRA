...
def computeCoordConnGLL(NEL, NGED, NGEL, varCoord, varCon,
    ...
    for ii in range(NED):
        ...
        halfCoord, edgePlane, edgeAngle, RE = computeEdgeParametersGLL(
            coord1, coord2)
    ...
    for ii in range(NEL):
        ...
        # Loop over the perimeter edges setting new interior nodes to the
        # connectivity
        cdex = range(1, NEEL)
        ...
        for jj in range(coords.shape[1]):
            ...
            for kk in range(1, NGED - 1):
                ...
                # Solve for the new coordinate
                sol = optimize.root(
                    findCoordAlongEdge, halfCoord, (edgePlane, coord1, newGridAngle, RE))
                # Store the new grid
                newGrid = np.zeros((3, 1))
                newGrid[:, 0] = sol.x
                varCoordGLL = np.append(varCoordGLL, newGrid, axis=1)
