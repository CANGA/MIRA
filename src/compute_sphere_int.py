...
with high accuracy. The public interface compute_sphere_int is a hybrid method
    ...
    for fid in range(nf):
        ...
        for j in range(1, nhe):
            ...
            for q in range(nqp):
                ...
                # weights x Jacobi
                w_j = ws[q] * tri_pro/(nrm_q**3)
                cell_int[fid]=cell_int[fid]+w_j*f_D(lon=sph_llq[0], lat=sph_llq[1])
                areas[fid] += w_j
...
def _cell_average_sphere_mix_split(xs, elems, f_D, deg=8):
    ...
    for fid in range(nf):
        ...
        if(h > tol):
            ...
            # recursive
            cell_int_fid, area_cell_fid = _cell_average_sphere_mix_split(pnts_vor, surf_fid[:(nhe-1)*4], f_D, deg)
            area_cell[fid] = sum(area_cell_fid)
            cell_int[fid] = sum(cell_int_fid)
        else:
            cell_int[fid], area_cell[fid] = _cell_average_sphere_mix2(xs, elems[fid].reshape((1, -1)), f_D, deg)
